...
class DFC2022(Dataset):
    ...
    def __init__(
        ...
        self.dem_min= -79.18
        self.dem_max =  3020.26
        self.dem_nodata = -99999.0
        self.patch_size = 256
        self.flag =flag
        self.crop_num = 8
        ...
        if self.transforms is not None:
            if self.flag =="train":
                ...
                for i in range(self.crop_num):
                    result.append(self.crop(sample))
            else:
                result = self.preprocess(sample)
...
class DFC2022DataSet:
    ...
    def train_label_dataloader(self):
        ...
        train_label_dataset = DFC2022(self.train_label_files, label=True, transforms=transforms)
        print('train_label_dataset', len(train_label_dataset))
        return DataLoader(train_label_dataset, batch_size=self.train_batch, num_workers=self.num_workers, shuffle=True, drop_last=True,collate_fn=self.collate_func_label)
    ...
    def train_unlabel_dataloader(self):
        ...
        transforms = T.Compose([self.preprocess, self.crop])
        train_unlabel_dataset = DFC2022(self.train_unlabel_files, label=False, transforms=transforms)
        print('train_unlabel_dataset', len(train_unlabel_dataset))
        return DataLoader(train_unlabel_dataset, batch_size=bs, num_workers=self.num_workers, shuffle=True, drop_last=True,collate_fn=self.collate_func_unlabel)

    def collate_func_label(self,batch_list):
        ...
        for i in range(len(batch_list)):
            ...
            for j in range(len(dic)):
                img_batch.append(dic[j]['image'])
                mask_batch.append(dic[j]['mask'])
        res ={}
        res["image"] = torch.stack(img_batch)
        res["mask"]  = torch.stack(mask_batch)
        return res
    def collate_func_unlabel(self,batch_list):
        batch_len=len(batch_list)
        #print(batch_len)
        img_batch =[]
        for i in range(len(batch_list)):
    ...
    def val_label_dataloader(self):
        ...
        val_label_dataset = DFC2022(self.val_label_files, label=True, transforms=transforms, image_size=(2000, 2000),flag ="val")
...
if __name__ == "__main__":
    ...
    dataset = DFC2022DataSet(dataset_cfg['ROOT_DIR'], dataset_cfg['TRAIN_BATCH'], dataset_cfg['VAL_BATCH'],
    ...
    train_label_loader = dataset.val_label_dataloader()
    labeled_trainiter = iter(train_label_loader)
    input_data_label = labeled_trainiter.next()
    print(type(input_data_label["image"]),len(input_data_label["image"]),input_data_label["image"].size())
